...
Proven delivery of production AI: agentic workflows and multi-agent architectures, MCP and API integrations across HTTP, SSE, and WebSocket transports, RAG, CAG, computer vision, multimodal generative AI across video, image, speech, and audio, and web3 tooling. End-to-end ownership from business requirements to deployment, in Agile cycles with stakeholders in the loop; architecture, integration tests, and security review stay human-in-the-loop.

1st place, Anna AI-Native App Hackathon 2026. Current verifiable experience is public: a live self-publishing media portal in production, more than ten published agent skills and toolkits, and open-source inference optimization on AMD hardware.
...
Compact Rust agent CLI for local models, with a GPU front end: concurrent sub-agents, plan mode, dockable window tabs and widgets, built-in hardware monitor (GPU, CPU, RAM, VRAM), activity and debug tooling, display themes, skills, MCP, kernel folder-lock (Landlock), and live context-budget accounting in a 4.3 MiB static binary.
...
| [gamentic](https://github.com/hec-ovi/gamentic) | The hackathon winner above, fully local. The world is an explicit state machine: everything the model changes goes through one of 40 validated tools writing to SQLite, so long adventures stay consistent. Every character is its own agent with bounded context and memory, and the text, image, and voice layers (llama.cpp, ComfyUI FLUX.2, Maya1) swap for any OpenAI-compatible or hosted vendor without touching the engine. |
| [noob&#8209;cli](https://github.com/hec-ovi/noob-cli) | Compact Rust agent CLI for local OpenAI-compatible models, with a GPU front end: concurrent sub-agents each in their own tab, plan mode with a live panel, dockable windows and widgets, built-in hardware monitor (GPU, CPU, RAM, VRAM), activity and debug reports that open any tool call, display themes, sessions, skills, MCP, web search, kernel folder-lock (Landlock), and live context-budget accounting with compaction receipts. 4.3 MiB static binary. |
...
| Project Repo Link | What it is |
|---|---|
| [telegram&#8209;bot&#8209;skill](https://github.com/hec-ovi/telegram-bot-skill) | Any local CLI coding agent (Claude Code, opencode, Codex, Gemini) as a private Telegram bot. One repo, three surfaces: installable skill, npm toolkit, MCP server over stdio or Streamable HTTP. Zero-dependency Node core, deterministic access tiers so strangers wait at a gate. |
| [agentickit](https://github.com/hec-ovi/agentickit) | React copilot framework on AG-UI: reads app state, fills forms, calls your tools, confirms destructive actions with the user before running them, `.pilot/` markdown skills. On npm. |
| [websearch&#8209;skill](https://github.com/hec-ovi/websearch-skill) | Keyless multi-engine web search and page reading for agents: rank fusion, clean Markdown extraction, and fetched content fenced as untrusted so a page cannot inject instructions into the agent. On PyPI. |
...
Two decisions carry most of an agent's accuracy: what the model is allowed to write, and what enters its context. At Ohara I removed hallucinated API code from Modu's generated apps by replacing free-written integrations with deterministic template injection: the model fills a validated interface it cannot deviate from. The same approach runs through the repos above: state-machine grounding in gamentic, source-checked publishing in Censurado, context budgets in noob-cli.

On the context side the corpus decides. CAG (cache-augmented generation) preloads a fixed corpus into the KV cache once and answers from it, which drops the retriever and the whole class of ranking and missed-chunk errors with it. That pays under specific conditions: the corpus is small enough to sit in the window comfortably, it is stable, and it is queried often enough that the cache actually gets reused, which is where the economics come from (reuse breaks even against a fresh prefill almost immediately). It stops paying when the corpus is large or churns, when it is per-tenant, when you owe citations, or when the context gets long enough that prefill cost and attention dilution eat the gain. In production this usually lands on a router: cache the hot stable part, retrieve the rest, and measure both paths. rag-base is where I keep the measurements, including the configurations where the reranker makes results worse.

| Project Repo Link | What it is |
|---|---|
| [rag&#8209;base](https://github.com/hec-ovi/rag-base) | Hybrid search backend: pgvector + ParadeDB BM25 + LightRAG graph, 4-mode rerank on CPU and GPU sidecars, GLiNER NER. 19 endpoints and an eval harness reporting hit@1 / hit@5 / MRR, including the configurations where the reranker makes results worse. |
...
Where the AI arc started. [computer-vision](https://github.com/hec-ovi/computer-vision) (2024) is my project for the computer-vision module of the Applied AI degree at IU: YOLOv8, EfficientDet, Detectron2, and SAM2 benchmarked head to head for object detection and segmentation in video, PyTorch on CUDA, with a React viewer for the results ([computer-vision-frontend](https://github.com/hec-ovi/computer-vision-frontend)). Two other modules on the degree have their own repo, both scored 100%: [healthhub](https://github.com/hec-ovi/healthhub) for object-oriented Python, a habit-tracking app (Flask backend, Node frontend, MongoDB, Docker), and [snakeless](https://github.com/hec-ovi/snakeless) for cloud programming, a Terraform-automated serverless stack on AWS: DynamoDB, two Lambda functions, and a static frontend served from S3 through CloudFront.

Further back: the early AI implementations I did at ADGS (Doha, 2018-2024), NLP before GPT was a product. Sentiment analysis, financial entity and number extraction compared across four engines (regex, spaCy, word2number, BERT), and tool calling implemented by hand, back when no model API offered it and LangChain did not exist. ADGS is also where I built a cybersecurity telemetry layer at the application frontend: vanilla JS that captured per-key timing, pressure, gyroscope, and location, and streamed it to a biometric-verification backend that decided whether you were really you. ADGS's flagship was Pandexit, an agent-based COVID-variant simulator the World Innovation Summit for Health named one of 15 promising 2020 health innovations; I engineered its dashboard end to end.

<sub>That dashboard: a vanilla-JS Leaflet map streaming live simulation state over WebSocket, an infection heatmap with moving per-agent markers, pie and time-series charts, and a JSON-configured engine that rendered the ABM's parameter forms on the fly, over the company's agent-based-modeling core. For position verification: the January 2021 press release where I am pictured as "Eng. Hector Oviedo" ([PRWeb](https://www.prweb.com/releases/deep-technology-startup-adgs-releases-pandexit-a-covid-variant-simulator-awarded-by-world-innovation-summit-for-health-894829283.html), [EIN Presswire](https://www.einpresswire.com/article/534798970/deep-technology-startup-adgs-releases-pandexit-a-covid-variant-simulator-awarded-by-world-innovation-summit-for-health), [EIN News](https://www.einnews.com/pr_news/534798970/deep-technology-startup-adgs-releases-pandexit-a-covid-variant-simulator-awarded-by-world-innovation-summit-for-health), [CCI France Qatar](https://www.ccifq.com/actualites/n/news/deep-technology-startup-adgs-releases-pandexit-a-covid-variant-simulator-awarded-by-world-innovation-summit-for-health.html)).</sub>
