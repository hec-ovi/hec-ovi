...
Compact Rust agent CLI for local models, with a GPU front end: kernel folder-lock (Landlock), skills, MCP, sub-agents, and live context-budget accounting in a 4.3 MiB static binary.
...
| [noob&#8209;cli](https://github.com/hec-ovi/noob-cli) | Compact Rust agent CLI for local OpenAI-compatible models, with a GPU front end: kernel folder-lock (Landlock), sessions, skills, MCP, plan mode, sub-agents, web search, and live context-budget accounting with compaction receipts. 1,700+ tests, 4.3 MiB static binary. |
